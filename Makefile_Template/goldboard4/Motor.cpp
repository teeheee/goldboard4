...
void Motor::rotateDirect(int16_t sp)
{
	if (sp == 0)
	{
		stop(false);
...
void Motor::accelerationHandler()
{
	if(sollspeed != istspeed)
	{
		if(sollspeed >= istspeed+motorAcceleration)
		{
			istspeed += motorAcceleration;
		}
		else if(sollspeed  <= istspeed-motorAcceleration)
		{
			istspeed -= motorAcceleration;
		}
		else
		...
		rotateDirect(istspeed);
	}
}

void Motor::setAcceleration(uint8_t percentPerHundretMs)
{
	motorAcceleration = percentPerHundretMs;
}
