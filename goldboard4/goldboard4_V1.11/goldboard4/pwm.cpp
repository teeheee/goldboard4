...
#define setBits(maske)	{MOTOR_PWM_PORT |= (maske & ALL_MOTOR_PINS); POWER_PWM_PORT |= ((maske >> 3) & ALL_POWER_PINS);}
#define clearBits(maske)	{MOTOR_PWM_PORT &= ~(maske & ALL_MOTOR_PINS); POWER_PWM_PORT &= ~((maske >> 3) & ALL_POWER_PINS);}

void copyData();
void sortData();
void calculateOCR();

uint8_t dutyCicles[PWM_PIN_COUNT] = { 0 };
uint8_t sortingPointer[PWM_PIN_COUNT]  = { 0 };

uint8_t OCRArray[ARRAY_DATA_SIZE] = { 0 };
uint8_t maskArray[ARRAY_DATA_SIZE] = { 0 };
...
uint8_t Testcase[6][6] = {{0,0,0,0,0,0},
